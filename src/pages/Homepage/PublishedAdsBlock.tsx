      ...
      <div className={`w-[85%] max-md:w-full flex flex-col gap-5 max-w-[${BreakPoint.xl}]`}>
        ...
        <div className="flex w-full gap-[1%] xl:flex-wrap xl:gap-y-3 max-w-full max-xl:overflow-x-scroll">
            {publishedAds.slice(0,10).map((item) => (
              <div onClick={()=>navigate(`/post-detail/${item.post.id}`)} key={item.id} className="cursor-pointer max-xl:w-[280px] w-[24.2%] relative border-2 rounded-2xl hover:-translate-y-1 hover:shadow-xl hover:bg-gray-50 duration-100">
                ...
                <div className="p-4 flex flex-col gap-3 w-full">
                  <div className="text-[20px] font-bold truncate">{item.post.title}</div>
